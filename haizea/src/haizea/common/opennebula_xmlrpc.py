...
class OpenNebulaXMLRPCClient(object):
	...
	self.auth = "%s:%s" % (user, password)
        ...
        self.socket.connect((OpenNebulaXMLRPCClient.TCP_IP,OpenNebulaXMLRPCClient.PORT))
        print "hello"
    ...
    def hostpool_info(self):
        try:
            (rc, value, other) = self.rpc.one.hostpool.info(self.auth)
            if rc == False:
    ...
    def host_info(self, hid):
        try:
            (rc, value, other) = self.rpc.one.host.info(self.auth, hid)
            if rc == False:
                raise Exception("ONE reported an error: %s" % value)
            else:
    ...
    def vmpool_info(self):
        try:
            (rc, value, other) = self.rpc.one.vmpool.info(self.auth, 0, -1, -1, -2) # -2: Get all VMs
            if rc == False:
                raise Exception("ONE reported an error: %s" % value)
            else:
    ...
    def vm_info(self, id):
        try:
            (rc, value, other) = self.rpc.one.vm.info(self.auth, id)
            if rc == False:
                raise Exception("ONE reported an error: %s" % value)
            else:
	...
	self.logger.info("%d deployed on %d" % (vid, hid))
        try:
            ...
            self.socket.send("DEPLOY:%d:%d" % (vid,hid))
            data = self.socket.recv(1024)
            res = data.split(":")
            if res[0] == "SUCCESS":
                return
            else:
                raise Exception("ONE reported an error: %s" % res[1])
        except xmlrpclib.Fault, err:
            raise Exception("XMLRPC fault: %s" % err.faultString)                    
...
class OpenNebulaHost(object):
	...
	self.storages = {}
	if len(self.template)!=0 and 'STORAGES' in self.template:
		for storage in self.template['STORAGES'].keys():						self.storages[storage]=self.template['STORAGES'][storage]
...
class OpenNebulaVM(object):
	...
	self.storages = []
	if len(self.template)!=0 and 'STORAGES' in self.template:
		for storage in self.template['STORAGES'].keys():
			self.storages.append(storage)
	
	self.clone = False
	if len(self.template)!=0 and 'DISK' in self.template:
		disk = self.template['DISK']
		if 'CLONE' in disk and disk['CLONE'] == 'yes':
			self.clone = True
		if 'SAVE' in disk:
			if disk['SAVE'] == 'yes':
				self.save = True
