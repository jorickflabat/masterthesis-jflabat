...
class OpenNebulaHaizeaVM(object):
	...
	#add storages of vm
	self.storages = opennebula_vm.storages

	#add clone
	self.clone = opennebula_vm.clone
	self.save = opennebula_vm.save
...
class OpenNebulaFrontend(RequestFrontend):    
    ...
    def __ONEreqs_to_lease(self, opennebula_vms, group_id=None):
        ...
        lease = Lease.create_new(submit_time = submit_time, 
				 ...
				 storages = vm_with_params.storages,
				 clone = vm_with_params.clone,
                                 save = vm_with_params.save)
